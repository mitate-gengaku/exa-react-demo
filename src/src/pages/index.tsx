...
import { ImageIcon, PictureInPictureIcon } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className={`${NotoSansJP.className} w-full p-6 lg:p-12`}>
      <form
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 lg:gap-8">
        {posts &&
          posts.results.map((result, i) => (
            <Link href={result.url} target="_blank" key={`${result.id}-${i}`}>
              <Card key={i} className="flex h-full flex-col">
                {result.meta && result.meta["og:image"] ? (
                  ...
                  <div className="flex h-56 w-full items-center justify-center rounded-md bg-gray-400">
                    <ImageIcon className="size-16 text-gray-50" />
                  </div>
                )}
                <CardHeader>
                  ...
                  <CardDescription>
                    {result.meta
                      ? readMore(result.meta["og:description"] || "")
                      : ""}
                  </CardDescription>
                </CardHeader>
